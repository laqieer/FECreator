...
def to_lab(rgb: np.ndarray) -> np.ndarray:
    ...
    return cv2.cvtColor(rgb32, cv2.COLOR_RGB2LAB).astype(np.float32)  # type: ignore[no-any-return]


def from_lab(lab: np.ndarray) -> np.ndarray:
    rgb32 = cv2.cvtColor(lab.astype(np.float32), cv2.COLOR_LAB2RGB)  # type: ignore[no-any-return]
